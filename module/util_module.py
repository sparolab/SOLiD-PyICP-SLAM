...
class UtilModule:
    ...
    def yawdeg2so3(self, yaw_deg):
        yaw_rad = np.deg2rad(yaw_deg)
        return self.eulerAnglesToRotationMatrix([0, 0, yaw_rad])

    def yawdeg2se3(self, yaw_deg):
        se3 = np.eye(4)
        se3[:3, :3] = self.yawdeg2so3(yaw_deg)
        return se3 
...
class PoseGraphResultSaver:
    ...
    def vizCurrentTrajectory(self, fig_idx, results):
        x = self.pose_list[:,3]
        y = self.pose_list[:,7]
        z = self.pose_list[:,11]
        ...
        plt.clf()
        plt.plot(-y, x, color='black') # kitti camera coord for clarity
        plt.axis('equal')
        ...
        # fig_idx = 2
        # plot.plot_trajectory(fig_idx, results, scale=20)
        # plt.xlim(-200, 200)
